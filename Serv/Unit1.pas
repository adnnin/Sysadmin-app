...
type
  TForm1 = class(TForm)
    ...
    procedure Button4Сlick(Sender: TObject);
...
begin
  if Assigned(ActiveClient) then  //если объект найден, то идем дальше
  begin
     ActiveClient.SendText('[Получить_ттх_пк]');
           //Показать кнопку Оценить ПК
...
begin
  ...
  Begin
    ActiveClient.SendText('[Перезагрузить_ПК]');

  End;
end;

procedure TForm1.Button4Сlick(Sender: TObject);
begin
       if Assigned (ActiveClient) then
  Begin
    ActiveClient.SendText('[Выключить_ПК]');
...
begin
//Полный объем физической памяти:   16 193 МБ
...
begin
  ...
  begin
    ...
    GroupBoxDataClient.Caption := 'Вы выбрали ПК - ' + Client.LocalHost;
...
procedure TForm1.ServerSocket1ClientRead(Sender: TObject;
  Socket: TCustomWinSocket);
begin
Memo1.Text := 'Получили ответ на запрос: ' + #13 + Socket.ReceiveText;
end;
